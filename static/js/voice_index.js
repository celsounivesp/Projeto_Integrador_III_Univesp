...
(function () {
  ...
  // (NOVO) Flag para desambiguação
  let autoListenAfterTTS = false;

  // --- 2. Funções de Áudio e UI ---

  /**
   * (MODIFICADO) Interrompe qualquer áudio que esteja tocando.
   */
  function stopTTS() {
    autoListenAfterTTS = false; // Cancela qualquer escuta pendente
    if (ttsPlayer) {
      ttsPlayer.pause();
      ttsPlayer.src = '';
    }
  ...
  /**
   * (MODIFICADO) Renderiza os cards de resultado, tela de erro, OU a desambiguação
   */
  function renderResults(data) {
    ...
    // (NOVO) Caso de Desambiguação (ex: "escola")
    if (data.ok && data.needs_desambiguation) {
      if (telaMic) telaMic.style.display = "block"; // Mantém o microfone visível
      resultsContainer.innerHTML = ""; // Limpa resultados antigos
      
      // Seta a flag. O áudio vai tocar (pelo 'handleSearch').
      // O listener 'onended' no final deste arquivo vai pegar essa flag
      // e chamar o onMicPress() automaticamente.
      autoListenAfterTTS = true; 
      return; // Não renderiza nada, só toca o áudio
    }
    ...
    if (data.ok && data.cards && data.cards.length > 0) {
      ...
      document.getElementById("btnRepetir").addEventListener("click", () => {
        // stopTTS() é chamado dentro de playTTS()
    ...
    // Se NÃO OK e é o erro 'show_call_button'
  ...
  function onMicPress(e) {
    ...
    // (CORREÇÃO DO BUG) Para o áudio de boas-vindas imediatamente
  ...
  // (NOVO) Listener para reabrir o microfone após desambiguação
  if (ttsPlayer) {
    ttsPlayer.addEventListener('ended', () => {
      if (autoListenAfterTTS) {
        autoListenAfterTTS = false; // Reseta a flag
        console.log("Áudio de desambiguação terminou. Reabrindo microfone...");
        onMicPress(null); // Chama a função de abrir o microfone
      }
    });
  }
